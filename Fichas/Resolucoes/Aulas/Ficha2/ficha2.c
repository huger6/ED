...
Elemento criar_elemento() {
    Elemento * p = (Elemento *) malloc(siezeof(Elemento));
    if (!p) return NULL;
...
void inserir_elemento_fim(Lista * lista, Elemento * e) {
    ...
    if (lista->n_elementos == 0) {
        ...
        //Chegámos ao último elemento da lista
        X->prox = e;
    }
...
Elemento * pesquisar_recursivo(Lista * li, Elemento * chave) {
    if (!li || !li->inicio) return NULL; //Não existe
...
void ordenarLista(Lista * li) {
    ...
    do {
        ...
        Elemento * p = atual->prox; //proximo
        trocou = 0;
        
        while (p) {
            if (comparar_elementos(a->info->texto, p->info->texto) > 0) {
